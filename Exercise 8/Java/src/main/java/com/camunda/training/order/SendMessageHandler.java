...
public class SendMessageHandler implements ExternalTaskHandler {
	...
	public void execute(ExternalTask externalTask, ExternalTaskService externalTaskService) {
	    ...
	    String paymentProcessInstanceId = "";
	    
		ApiClient client = new ApiClient();
	    
	    try {
	    	
	    	List<MessageCorrelationResultWithVariableDto> correlationResult = new MessageApi(client).deliverMessage(new CorrelationMessageDto().messageName("paymentRequestMessage").businessKey(externalTask.getBusinessKey()).processVariables(payload).resultEnabled(true));
